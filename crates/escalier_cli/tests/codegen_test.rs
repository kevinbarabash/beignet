...
fn number_literal_real() {
    insta::assert_snapshot!(compile("5.0"), @"5;
");
}
...
fn lambda_with_two_args() {
    insta::assert_snapshot!(compile("fn (a, b) => a + b"), @"(a, b)=>a + b;
");
}

#[test]
fn lambda_with_one_arg() {
    insta::assert_snapshot!(compile("fn (a) => a"), @"(a)=>a;
");
}

#[test]
fn lambda_with_no_args() {
    insta::assert_snapshot!(compile("fn () => 5"), @"()=>5;
");
}
...
fn function_declaration() {
    insta::assert_snapshot!(compile("let add = fn (a, b) => a + b"), @"export const add = (a, b)=>a + b;
");
}
...
fn let_in_inside_declaration() {
    insta::assert_snapshot!(compile("let foo = do {let x = 5\nx}"), @r###"
    let $temp_0;
    {
        const x = 5;
        $temp_0 = x;
...
fn js_print_nested_lambdas() {
    insta::assert_snapshot!(compile("let foo = fn (a) => fn (b) => a + b"), @"export const foo = (a)=>(b)=>a + b;
");
}

#[test]
fn js_print_nested_lambdas_with_multiple_lines() {
    insta::assert_snapshot!(compile("let foo = fn (a) => fn (b) {let sum = a + b\nreturn sum}"), @r###"
